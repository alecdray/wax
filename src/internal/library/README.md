...
`library` owns the user's relationship to albums — what's in their collection, what physical formats they own, what's on their watch list, what's been recently played. The album is the central aggregate: an `AlbumDTO` composes data from peer modules (ratings, tags, primary genres, sleeve notes, last-played) into one shape that the album view UI binds to. Primary genres come from the [genres module](../genres/README.md) and drive both read-only badges and the dashboard's genre filter.

`library` also owns the **album view UI** — every user-facing surface that centres on albums, including the dashboard, album-detail page, the radar page (with its Spotify album-search affordance), and the modals that mutate album state. It also owns the **app header** rendered on every authenticated library page: a slim top bar with the wax wordmark and the feed sync-status control (navigation itself lives in the shared bottom nav — see [ADR 0007](../../../docs/adr/0007-bottom-nav-all-viewports.md)). When peer modules (review, tags) mutate album state, they broadcast an `album-changed` HTMX event; a hidden listener in that app header responds by calling `GET /app/library/album-surfaces`, which re-renders the affected album surfaces as OOB swaps.
