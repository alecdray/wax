...
The lifecycle state is a pure function of the save action, applied from any prior state:

- **Save** lands the album in `provisional`. It creates the state row on an album's first rating and demotes a `finalized` album back to `provisional` — saving is the only path that un-finalizes.
- **Finalize** lands the album in `finalized`, whether it was unrated, provisional, or already finalized.

Both actions append a rating-log entry with the entered score. The resulting state depends only on which action was taken, never on where the album started.
