...
func RegisterRoutes(mux *httpx.Mux, h *HttpHandler) {
	...
	mux.Handle("POST /app/review/rating-recommender/rating/finalize", httpx.HandlerFunc(h.SubmitRatingRecommenderFinalize))
	mux.Handle("POST /app/review/rating-recommender/confirm", httpx.HandlerFunc(h.GetRatingConfirm))
	mux.Handle("GET /app/review/rating-recommender/confirm", httpx.HandlerFunc(h.GetRatingConfirm))
