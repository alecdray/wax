// Package genregraph builds and queries the Wikidata-derived music genre DAG:
// fuzzy lookup, ancestor/descendant traversal, and the curated primary-genre
// allowlist used to map a sub-genre to its broad buckets. See ADR 0009.
package genregraph
...
// DAG is a directed acyclic graph of music genregraph.
type DAG struct {
	nodes map[string]*Node
	Root  *Node

	// primaries holds each node's most-specific primary genres, precomputed at
	// Build time so reads are lock-free on the immutable DAG.
