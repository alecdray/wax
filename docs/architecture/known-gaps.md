...
## E2E suite shares a single SQLite DB with no per-spec isolation

All specs run against a shared SQLite file with no per-test or per-spec teardown. `playwright.config.ts` pins `workers: 1` to keep the suite deterministic, which works but gives up parallelism. The right fix is WAL + `busy_timeout` on the DSN combined with per-spec test data isolation (separate DB per worker or explicit setup/teardown), then lifting the `workers: 1` pin. Until then, any spec that mutates state must be aware of what prior specs may have left behind.
