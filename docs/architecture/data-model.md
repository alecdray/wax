...
- **Ratings are an append-only log plus a state machine.** Every rating event is recorded with its note; the album's current lifecycle position (provisional / finalized / snoozed, next-rerate timing) is tracked separately. See the [review module](../../src/internal/review/README.md) for the rating philosophy.
