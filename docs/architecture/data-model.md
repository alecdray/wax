...
- **Crates are named, unordered collections.** A user groups albums into crates by personal criteria ("jazz," "gifts for friends"). Crates are first-class objects with their own pages — distinct from tags (inline labels) and ranked lists (where position is the point). An album can belong to multiple crates; crates carry no position metadata.
- **Genres are app-curated, not user-authored.** Distinct from tags: genres come from a shared, app-owned taxonomy (a curated set of *primary* genre buckets backed by the Wikidata genre graph), and an album's genres are *derived* by auto-assignment, not typed by the user. The "no global taxonomy" rule above is about tags; genres are the deliberate exception.
- **Annotations are all optional.** The library is valuable without ratings, tags, notes, genres, or radar — each layer is additive.
